...
use image::{DynamicImage, GenericImageView, ImageBuffer, ImageReader, imageops};
...
impl ImageProcessor {
    ...
    pub fn set_mask(
        ...
        if mask_image.width() != dimensions.size || mask_image.height() != dimensions.size {
            return Err(JsValue::from_str(
                "Mask image size does not match dimensions",
            ));
        }
    ...
    pub fn draw_on_mask(
        ...
        imageproc::drawing::draw_filled_circle_mut(img, (x, y), radius, color);
    ...
    pub fn render(
        ...
        let stenciled_image = ImageBuffer::from_fn(dimensions.size, dimensions.size, |x, y| {
            ...
            let is_masked = self
                .mask_image
                .as_ref()
                .map_or(false, |mask| mask.get_pixel(x, y).0[3] > 0);
        ...
        image::imageops::overlay(&mut composite_image, &stenciled_image, 0, 0);
